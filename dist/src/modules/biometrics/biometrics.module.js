"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.BiometricsModule = void 0;
const common_1 = require("@nestjs/common");
const biometrics_service_1 = require("./biometrics.service");
const biometrics_controller_1 = require("./biometrics.controller");
const biometrics_authorization_service_1 = require("./biometrics.authorization.service");
const coordinates_service_1 = require("./coordinates.service");
let BiometricsModule = class BiometricsModule {
};
BiometricsModule = __decorate([
    (0, common_1.Module)({
        controllers: [biometrics_controller_1.BiometricsController],
        providers: [coordinates_service_1.CoordinatesService, biometrics_service_1.BiometricsService, biometrics_authorization_service_1.BiometricsAuthorizationService]
    })
], BiometricsModule);
exports.BiometricsModule = BiometricsModule;
//# sourceMappingURL=biometrics.module.js.map